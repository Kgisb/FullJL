...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report", "Pipeline", "Sales Activity", "Deal stage", "Original source", "Referral / No-Referral", "Lead mix", "Referral performance", "Slow Working Deals", "Activity concentration" ],
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis" ],
    }
...
def _render_marketing_master_analysis(
    ...
    # ---- Cascading column/value filters ----
    ...
    # Build value filters for each selected column — values derived from the already globally filtered df_f
    value_selections: dict[str, list] = {}
    d0 = df_f.copy()
    if selected_cols:
        ...
        for c in selected_cols:
            ...
            chosen_clean = [v for v in chosen if v != "All"]
            if not chosen_clean:
                value_selections[c] = ["__ALL__"]
            else:
                value_selections[c] = chosen_clean

        # Apply value filters
        for c in selected_cols:
            chosen = value_selections.get(c, ["__ALL__"])
            if "__ALL__" in chosen:
                continue
            d0 = d0[d0[c].astype(str).isin(chosen)]

    # ---- Date parsing (robust, day-first) ----
    create_dt = coerce_datetime(d0[_create]).dt.date if _create in d0.columns else pd.Series(pd.NaT, index=d0.index)
    pay_dt    = coerce_datetime(d0[_pay]).dt.date    if (_pay and _pay in d0.columns) else pd.Series(pd.NaT, index=d0.index)
    first_dt  = coerce_datetime(d0[_first]).dt.date  if (_first and _first in d0.columns) else pd.Series(pd.NaT, index=d0.index)
    resch_dt  = coerce_datetime(d0[_resch]).dt.date  if (_resch and _resch in d0.columns) else pd.Series(pd.NaT, index=d0.index)
    done_dt   = coerce_datetime(d0[_done]).dt.date   if (_done  and _done  in d0.columns) else pd.Series(pd.NaT, index=d0.index)

    # ---- Window masks ----
    c_in = create_dt.between(start_d, end_d)
    p_in = pay_dt.between(start_d, end_d)       if _pay   else pd.Series(False, index=d0.index)
    f_in = first_dt.between(start_d, end_d)     if _first else pd.Series(False, index=d0.index)
    r_in = resch_dt.between(start_d, end_d)     if _resch else pd.Series(False, index=d0.index)
    d_in = done_dt.between(start_d, end_d)      if _done  else pd.Series(False, index=d0.index)
    ...
    st.dataframe(table, use_container_width=True, hide_index=True)
    st.download_button(
        "Download CSV — Master analysis",
        data=table.to_csv(index=False).encode("utf-8"),
        file_name="marketing_master_analysis.csv",
        mime="text/csv",
        key="dl_master_analysis_csv"
    )

    # ---- Optional: grouped breakdown by selected columns (toggle) ----
    show_breakdown = st.toggle("Show grouped breakdown by selected columns", value=False, key="ma_show_grp")
    if show_breakdown and selected_cols:
        grp_cols = [c for c in selected_cols if c in d0.columns]
        if grp_cols:
            g = d0.copy()
            ...
            gc = _cnt(g["_create_dt"].between(start_d, end_d))

            if mode == "MTD":
                ge = _cnt(((_pay is not None)   and g["_pay_dt"].between(start_d, end_d))   & g["_create_dt"].between(start_d, end_d)) if _pay   else pd.Series(dtype=int)
                gf = _cnt(((_first is not None) and g["_first_dt"].between(start_d, end_d)) & g["_create_dt"].between(start_d, end_d)) if _first else pd.Series(dtype=int)
                gr = _cnt(((_resch is not None) and g["_resch_dt"].between(start_d, end_d)) & g["_create_dt"].between(start_d, end_d)) if _resch else pd.Series(dtype=int)
                gd = _cnt(((_done is not None)  and g["_done_dt"].between(start_d, end_d))  & g["_create_dt"].between(start_d, end_d)) if _done  else pd.Series(dtype=int)
            else:
                ge = _cnt((_pay   is not None)  and g["_pay_dt"].between(start_d, end_d))   if _pay   else pd.Series(dtype=int)
                gf = _cnt((_first is not None)  and g["_first_dt"].between(start_d, end_d)) if _first else pd.Series(dtype=int)
                gr = _cnt((_resch is not None)  and g["_resch_dt"].between(start_d, end_d)) if _resch else pd.Series(dtype=int)
                gd = _cnt((_done  is not None)  and g["_done_dt"].between(start_d, end_d))  if _done  else pd.Series(dtype=int)
            ...
            if not gr.empty: out = out.merge(gr.rename("Trial Rescheduled"), left_index=True, right_index=True, how="left")
            ...
            st.dataframe(out, use_container_width=True)
            st.download_button(
                "Download CSV — Master analysis (grouped)",
                data=out.to_csv(index=False).encode("utf-8"),
                file_name="marketing_master_analysis_grouped.csv",
                mime="text/csv",
                key="dl_master_analysis_grouped_csv"
            )



# --- Router: Marketing → Master analysis (appended by patcher) ---
try:
    _master_for_ma = master if 'master' in globals() else st.session_state.get('nav_master', '')
    _view_for_ma = st.session_state.get('nav_sub', locals().get('view', ''))
...
def _render_marketing_master_analysis(
    ...
    # Convert to pandas Timestamps (normalize to day)
    ...
    # ---- Date parsing (keep datetime64[ns]) ----
    create_dt = coerce_datetime(d0[_create]) if _create in d0.columns else pd.to_datetime(pd.Series([None]*len(d0)))
    pay_dt    = coerce_datetime(d0[_pay])    if (_pay and _pay in d0.columns)    else pd.to_datetime(pd.Series([None]*len(d0)))
    first_dt  = coerce_datetime(d0[_first])  if (_first and _first in d0.columns) else pd.to_datetime(pd.Series([None]*len(d0)))
    resch_dt  = coerce_datetime(d0[_resch])  if (_resch and _resch in d0.columns) else pd.to_datetime(pd.Series([None]*len(d0)))
    done_dt   = coerce_datetime(d0[_done])   if (_done  and _done  in d0.columns) else pd.to_datetime(pd.Series([None]*len(d0)))

    # Normalize to day (midnight) for consistent between()
    create_dt = create_dt.dt.floor("D")
    if _pay:   pay_dt   = pay_dt.dt.floor("D")
    if _first: first_dt = first_dt.dt.floor("D")
    if _resch: resch_dt = resch_dt.dt.floor("D")
    if _done:  done_dt  = done_dt.dt.floor("D")

    # ---- Window masks (vectorized) ----
    ...
    if show_breakdown and selected_cols:
        ...
        if grp_cols:
            ...
            # Helper to safely build counts per group for a mask
            ...
            if mode == "MTD":
                m_enrol = (g["_pay_dt"].between(start_ts, end_ts) & m_create) if _pay else None
                m_first = (g["_first_dt"].between(start_ts, end_ts) & m_create) if _first else None
                m_resch = (g["_resch_dt"].between(start_ts, end_ts) & m_create) if _resch else None
                m_done  = (g["_done_dt"].between(start_ts, end_ts) & m_create) if _done else None
            else:
                m_enrol = (g["_pay_dt"].between(start_ts, end_ts))   if _pay else None
                m_first = (g["_first_dt"].between(start_ts, end_ts)) if _first else None
                m_resch = (g["_resch_dt"].between(start_ts, end_ts)) if _resch else None
                m_done  = (g["_done_dt"].between(start_ts, end_ts))  if _done else None
...
def _render_marketing_master_analysis(
    ...
    # Safe comparator handling: try datetime64 vs Timestamp; if it fails, compare .dt.date
    def _between(series: pd.Series, start_ts, end_ts, start_d, end_d):
        try:
            return series.between(start_ts, end_ts)
        except Exception:
            try:
                ser_date = series.dt.date
                return ser_date >= start_d if start_d == end_d else ser_date.between(start_d, end_d)
            except Exception:
    ...
    if selected_cols:
        ...
        for c in selected_cols:
            ...
            chosen = st.multiselect(f"{c} values", options=opts, default=["All"], key=f"ma_vals_{c}")
            chosen_clean = [v for v in chosen if v != "All"]
            value_selections[c] = chosen_clean if chosen_clean else ["__ALL__"]

        for c in selected_cols:
            chosen = value_selections.get(c, ["__ALL__"])
            if "__ALL__" in chosen: continue
            d0 = d0[d0[c].astype(str).isin(chosen)]
    ...
    # Normalize to day (midnight) for consistent window comparisons
    create_dt = create_dt.dt.floor("D")
    if _pay:   pay_dt   = pay_dt.dt.floor("D")
    if _first: first_dt = first_dt.dt.floor("D")
    if _resch: resch_dt = resch_dt.dt.floor("D")
    ...
    # ---- Window masks (safe) ----
    c_in = _between(create_dt, start_ts, end_ts, start_d, end_d)
    p_in = _between(pay_dt,    start_ts, end_ts, start_d, end_d) if _pay   else pd.Series(False, index=d0.index)
    f_in = _between(first_dt,  start_ts, end_ts, start_d, end_d) if _first else pd.Series(False, index=d0.index)
    r_in = _between(resch_dt,  start_ts, end_ts, start_d, end_d) if _resch else pd.Series(False, index=d0.index)
    d_in = _between(done_dt,   start_ts, end_ts, start_d, end_d) if _done  else pd.Series(False, index=d0.index)
    ...
    if show_breakdown and selected_cols:
        ...
        if grp_cols:
            ...
            m_create = _between(g["_create_dt"], start_ts, end_ts, start_d, end_d)

            if mode == "MTD":
                m_enrol = (_between(g["_pay_dt"], start_ts, end_ts, start_d, end_d) & m_create) if _pay else None
                m_first = (_between(g["_first_dt"], start_ts, end_ts, start_d, end_d) & m_create) if _first else None
                m_resch = (_between(g["_resch_dt"], start_ts, end_ts, start_d, end_d) & m_create) if _resch else None
                m_done  = (_between(g["_done_dt"], start_ts, end_ts, start_d, end_d) & m_create) if _done else None
            else:
                m_enrol = _between(g["_pay_dt"], start_ts, end_ts, start_d, end_d)   if _pay else None
                m_first = _between(g["_first_dt"], start_ts, end_ts, start_d, end_d) if _first else None
                m_resch = _between(g["_resch_dt"], start_ts, end_ts, start_d, end_d) if _resch else None
                m_done  = _between(g["_done_dt"], start_ts, end_ts, start_d, end_d)  if _done else None
