...
def _render_marketing_master_analysis(
    ...
    # Normalize bounds to timezone-naive Timestamps @ midnight
    start_ts = pd.Timestamp(start_d).normalize().tz_localize(None)
    end_ts   = pd.Timestamp(end_d).normalize().tz_localize(None)

    # Helper: convert any series to timezone-naive datetime64[ns] floored to day
    ...
    # ---- Date parsing (strict, tz-naive timestamps) ----
    ...
    # ---- Window masks (Timestamp vs Timestamp) ----
...
# --- Router: Marketing → Master analysis (FIX3) ---
try:
...
def _render_marketing_master_analysis(
    ...
    d0 = df_f.copy()
    # Track which columns have narrowed selections (specific values, not "All")
    narrowed_cols = []
    ...
    create_dt = _to_day_ts(d0[_create]) if _create in d0.columns else pd.Series(pd.NaT, index=d0.index)
    pay_dt    = _to_day_ts(d0[_pay])    if (_pay and _pay in d0.columns)    else pd.Series(pd.NaT, index=d0.index)
    first_dt  = _to_day_ts(d0[_first])  if (_first and _first in d0.columns) else pd.Series(pd.NaT, index[d0.index] if hasattr(d0, 'index') else None)
    resch_dt  = _to_day_ts(d0[_resch])  if (_resch and _resch in d0.columns) else pd.Series(pd.NaT, index=d0.index)
    done_dt   = _to_day_ts(d0[_done])   if (_done  and _done  in d0.columns) else pd.Series(pd.NaT, index=d0.index)
    ...
    # ---- Row-wise grouped breakdown when multiple values are selected ----
    auto_breakdown = len(narrowed_cols) > 0  # any column had specific values
    ...
    if show_breakdown and (selected_cols or narrowed_cols):
        # Group by only the columns where the user narrowed values; if none, group by selected columns
