...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Ref_Tracker" ],
    }
...
def _render_marketing_ref_tracker(
    ...
    try:
        ...
        st.info(f"Pie chart unavailable ({type(_pie_err).__name__}). Showing counts instead.")
        st.write({"Within": within_count, "Beyond": beyond_count, "Remaining": max(total_count - (within_count + beyond_count), 0)})


    # Sanity: A+B==Total
    if within_count + beyond_count != total_count:
        st.warning(f"Partition mismatch: A({within_count}) + B({beyond_count}) != Total({total_count}). Please review date columns.")
