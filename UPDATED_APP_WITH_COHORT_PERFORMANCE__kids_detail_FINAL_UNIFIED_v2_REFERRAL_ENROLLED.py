...
def _render_marketing_referral_tracking(
    ...
    table = pd.DataFrame([{
        ...
        "Deal → Enrollment % (All)": round(conv_all, 1) if pd.notna(conv_all) else np.nan,
        ...
        "Deal → Enrollment % (Parent Enrolled)": round(conv_enrolled, 1) if pd.notna(conv_enrolled) else np.nan,
    ...
    if show_by_email:
        ...
        # Enrolled? + First enrollment date columns
